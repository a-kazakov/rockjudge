...
export default class UniversalSelector extends React.Component {
    ...
    renderAdminButton() {
        ...
        return (
            <a className="mbtn no-title" href={this.admin_href}>
                {_("start_page.roles.administrator")}
            </a>
        );
    }
    render() {
        const all_judges = this.props.competition.judges.filter(
            judge => judge.discipline_judges.length > 0,
        );
        const line_judges = all_judges
            .filter(judge => judge.role_description === "")
            .map(judge => (
                <a className="mbtn" href={this.getJudgeRef(judge)} key={judge.id}>
                    <div className="title">
        ...
        const staff = all_judges
            ...
            .map(judge => (
                <a className="mbtn" href={this.getJudgeRef(judge)} key={judge.id}>
                    <div className="title">{judge.role_description}</div>
                    <div className="name">{judge.name}</div>
                </a>
            ));
        return (
            <div className="universal-selector">
                <div className="content">
                    <div className="group">
                        <div className="btn-group-vertical full-width">{staff}</div>
                    </div>
                    <div className="group">
                    ...
                    <div className="group">
                        <div className="btn-group-vertical full-width">
                            ...
                            <a className="mbtn no-title" href={this.presenter_href}>
                            ...
                            <a className="mbtn no-title" href={this.screen_href}>
