...
export default class TechAcroLayout extends React.Component {
    ...
    constructor(props) {
        ...
        if (this.getValue("judging_position", true, pre_state, props) == null) {
            ...
            pre_state.heat =
                this.getValue("first_non_confirmed_heat", true, pre_state, props) ||
    ...
    get judging_position() {
        return this.getValue("judging_position");
    }
    get interesting_runs() {
        return this.getValue("interesting_runs");
    }
    get score_by_run_id() {
        return this.getValue("score_by_run_id");
    }
    get heats_count() {
        return this.getValue("heats_count");
    }
    get first_non_confirmed_heat() {
        return this.getValue("first_non_confirmed_heat");
    }
    get can_finish() {
        return this.getValue("can_finish");
    }
    get current_run() {
        return this.getValue("current_run");
    }
    get current_score() {
        return this.getValue("current_score");
    }
    get max_runs_per_heat() {
        return this.getValue("max_runs_per_heat");
    }
    ...
    getScoreByRunId(context) {
        function findScore(run) {
            return (
                run.scores.find(
                    s => s.discipline_judge_id === context.props.disciplineJudge.id,
                ) || null
            );
        }
    ...
    getCurrentRun(context) {
        return (
            this.getValue("interesting_runs", context).find(
                run => run.heat === this.state.heat,
            ) || null
        );
    }
    ...
    handleScoreUpdate = (key, value, force = false) => {
        ...
        const score_data = key == null ? value : { [key]: value };
    ...
    handleJudgingPositionSelected = judgingPosition => {
        ...
        state_upd.heat =
            this.getValue("first_non_confirmed_heat", true, next_state) ||
    ...
    renderBody() {
        ...
        if (this.state.showLastPage) {
            return <LastPage tour={this.props.tour} />;
        }
        ...
        if (this.current_score == null) {
            return (
                <div className="body not-performing">{_("tablet.global.no_score")}</div>
            );
        }
        return (
            <div className="body">
        ...
        return (
            <div className="vftsarr-JudgeTablet TechJudgeAcroLayout">
                <Header
                    ...
                    maxHeat={
                        this.judging_position == null
                            ? 1
                            : this.first_non_confirmed_heat || this.heats_count
                    }
                    tour={this.props.tour}
