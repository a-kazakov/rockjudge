...
import operator
import random
from typing import Any, Dict, Iterable, NamedTuple, Optional, Set, TYPE_CHECKING, Tuple, Union
...
class Client(ModelBase, BaseModel):
    ...
    def verification_string(self) -> str:
        if self.secret is None:
            raise InternalError("Unable to get verification string since there is no secret")
    ...
    def get_and_validate(
        ...
        correct_sig = hashlib.md5(correct_sig_src.encode("utf-8")).hexdigest().encode("utf-8")
        user_sig = signature.encode("utf-8")
        if (
                # Time-constant comparison
                len(user_sig) != len(correct_sig)
                or functools.reduce(operator.or_, (a ^ b for a, b in zip(correct_sig, user_sig))) != 0
        ):
