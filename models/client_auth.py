...
class ClientAuth(ModelBase, BaseModel):
    ...
    access_level = Column(EnumColumn(AccessLevel), nullable=False, default=AccessLevel.NONE)
    ...
    def before_update(self, data: Dict[str, Any], *, unsafe: bool) -> Dict[str, Any]:
        ...
        if "judge_id" in data:
            if data["judge_id"] is None:
                ...
                judge = self.session.query(Judge).filter_by(
    ...
    @classmethod
    def get_for_competition(cls, session: Session, client: "Client", competition_id: int) -> "ClientAuth":
        auth = session.query(ClientAuth).filter_by(
            client_id=client.id,
