...
export default class TourScoresWrapper {
    constructor(tour, results) {
        this.run_wrappers = tour.runs.map(
            run => new RunScoresWrapper(run, tour.discipline_judges),
        );
        this.discipline_judges = tour.discipline.discipline_judges;
        this.discipline_judges_by_roles = {};
        this.discipline_judges.forEach(
            function(dj, idx) {
        ...
        if (results) {
            ...
            results.forEach(res => (results_by_run_ids[res.run_id] = res));
            this.run_wrappers.forEach(
                w => (w.results_info = results_by_run_ids[w.run.id]),
            );
            this.run_wrappers.sort(
                (a, b) => a.results_info.place - b.results_info.place,
            );
        }
    }
    ...
    getScoresTableByRoles(...args) {
        const discipline_judge_ids = this.getDisciplineJudgesByRoles(...args).map(
            dj => dj.id,
        );
