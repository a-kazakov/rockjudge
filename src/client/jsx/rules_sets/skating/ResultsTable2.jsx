...
export default class ResultsTable2 extends React.Component {
    ...
    setupCache() {
        ...
        this.line_judges = tour.discipline.discipline_judges.filter(
        ...
        this.show_skating_table =
            this.constructor.verbose && tour.scoring_system_name.includes("final");
        this.show_ec_table =
    ...
    getCols() {
        ...
        if (this.show_tours_places && this.table.length > 0) {
            const prev_tours_count = this.table[0].run_result.extra_data.tours_places
                .length;
    ...
    getRows() {
        ...
        for (let idx = 0; idx < this.table.length; ++idx) {
            ...
            const scores_map = new Map(
                row.run.scores.map(score => [score.discipline_judge_id, score]),
            );
            for (const dj of this.line_judges) {
                row_data[`judge_score_${dj.id}`] = this.getScoreText(
                    row,
                    scores_map.get(dj.id),
                );
            }
