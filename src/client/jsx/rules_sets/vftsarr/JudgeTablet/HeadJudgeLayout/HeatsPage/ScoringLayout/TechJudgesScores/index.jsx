...
export default class TechJudgeScore extends React.Component {
    ...
    setupCache() {
        this.tech_judges = this.props.disciplineJudges.filter(
            dj => dj.role === "tech_judge",
        );
        this.tech_judges_index = new Map(this.tech_judges.map(dj => [dj.id, dj]));
        this.scores = this.props.run.scores.filter(score =>
            this.tech_judges_index.has(score.discipline_judge_id),
        );
    }
    ...
    render() {
        ...
        return (
            <div>
                ...
                <table className="tech-judge-scores" style={this.getStyle()}>
                    <tbody>
